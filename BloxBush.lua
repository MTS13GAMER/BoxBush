...
local FightingStyleTab = Window:CreateTab("Fighting Style")
...
local World1Section = MainTab:CreateSection("World 1")
local World2Section = MainTab:CreateSection("World 2")
local World3Section = MainTab:CreateSection("World 3")
local GodHumanSection = MainTab:CreateSection("For God Human")

local FightingStyleSection = FightingStyleTab:CreateSection("Fighting Styles")

local BossSection = BossTab:CreateSection("Boss Options")

local MasterySection = MasteryTab:CreateSection("Mastery Options")

local ConfigsGeneralSection = ConfigsTab:CreateSection("General")
local ConfigsSkillSection = ConfigsTab:CreateSection("Skill Configs")
local ConfigsMobSection = ConfigsTab:CreateSection("Mob Configs")

local StatsSection = StatsTab:CreateSection("Auto Stats")
local RedeemSection = StatsTab:CreateSection("Auto Redeem Code")

local MiscGeneralSection = MiscTab:CreateSection("General")
local MiscServerSection = MiscTab:CreateSection("Server")

local TeleportSection = TeleportTab:CreateSection("Teleports")

local FruitsSection = FruitsTab:CreateSection("Devil Fruits")

local RaidsSection = RaidsTab:CreateSection("Raids")

local CombatSection = CombatTab:CreateSection("Aimbot")

local HUDSection = HUDTab:CreateSection("HUD Settings")
		...
		if _G.AutoFarmLevelReal then
			if QuestC.Visible == true then
				...
				if game:GetService("Workspace").Enemies:FindFirstChild(QuestCheck()[3]) then
							...
							if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
								repeat task.wait()
									if not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, QuestCheck()[6]) then
										...
										v.Head.CanCollide = false
										BringMobFarm = true
										EquipWeapon(_G.Settings.Configs["Select Weapon"][1] or "Combat")
										v.HumanoidRootPart.Transparency = 1
					...
					UnEquipWeapon(_G.Settings.Configs["Select Weapon"][1] or "Combat")
					if QuestCheck()[7] and QuestCheck()[7][SetCFarme] then
				...
				if game:GetService("Players").LocalPlayer.Data.LastSpawnPoint.Value == tostring(GetIsLand(QuestCheck()[7][1])) then
					...
					if QuestCheck()[7] and QuestCheck()[7][1] then
...
GeneralSection:CreateSlider({
	...
	Min = 100,
	Max = 5000,
	Default = _G.Settings.Main["Distance Mob Aura"],
	Callback = function(Value)
...
-- Configs Tab
ConfigsGeneralSection:CreateToggle({
	Name = "Fast Attack",
...
ConfigsGeneralSection:CreateDropdown({
	Name = "Fast Attack Type",
	Options = {"Fast","Normal","Slow"},
	Default = _G.Settings.Configs["Fast Attack Type"][1],
	Callback = function(Value)
...
-- Adicione mais elementos de interface para outras seções conforme necessário

print("Blox Fruits GUI loaded successfully!")
